...
if buildAll || hasArg libcudf; then
    ...
    cmake_hpcc -S "$REPODIR"/cpp -B "${LIB_BUILD_DIR}" \
    ...
    cmake_hpcc --build . -j"${PARALLEL_LEVEL}" ${VERBOSE_FLAG}
    compile_end=$(date +%s)
    ...
    if [[ ${INSTALL_TARGET} != "" ]]; then
        cmake_hpcc --build . -j"${PARALLEL_LEVEL}" --target install ${VERBOSE_FLAG}
    fi
fi
...
if hasArg libcudf_kafka; then
    cmake_hpcc -S "$REPODIR/cpp/libcudf_kafka" -B "${KAFKA_LIB_BUILD_DIR}" \
          -DCMAKE_INSTALL_PREFIX="${INSTALL_PREFIX}" \
          -DBUILD_TESTS=${BUILD_TESTS} \
          -DCMAKE_BUILD_TYPE=${BUILD_TYPE} \
          "${EXTRA_CMAKE_ARGS[@]}"
