...
if buildAll || hasArg libcudf; then
    ...
    cmake_maca -S "$REPODIR"/cpp -B "${LIB_BUILD_DIR}" \
    ...
    cmake_maca --build . -j"${PARALLEL_LEVEL}" ${VERBOSE_FLAG}
    compile_end=$(date +%s)
    ...
    if [[ ${INSTALL_TARGET} != "" ]]; then
        cmake_maca --build . -j"${PARALLEL_LEVEL}" --target install ${VERBOSE_FLAG}
    fi
fi
...
if hasArg libcudf_kafka; then
    cmake_maca -S "$REPODIR/cpp/libcudf_kafka" -B "${KAFKA_LIB_BUILD_DIR}" \
          -DCMAKE_INSTALL_PREFIX="${INSTALL_PREFIX}" \
          -DBUILD_TESTS=${BUILD_TESTS} \
          -DCMAKE_BUILD_TYPE=${BUILD_TYPE} \
          "${EXTRA_CMAKE_ARGS[@]}"
